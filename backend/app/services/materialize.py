...
3. 字段：title=裸字符串；skill_key=该 week focus_skills[0]（roadmap 不暴露技能 key，
   ...
   planned_date：周首日 = base_date + (week-1)*7，再按周内序 order_index 把任务摊到本周
   0..6 天（E1.1：day_offset = min(6, order_index*7 // 周内任务数)），避免整周挤在首日。
   base_date = journey.start_date or run.created_at.date()。
   注：物化只设「初始」每日排期；之后由 replan(E1.2) 按完成情况顺延/重组，
   仅显式重新生成计划会重置回此初始分布。
...
def materialize_tasks(
    ...
    for week_item in roadmap:
        ...
        week_start = base_date + timedelta(days=(week - 1) * 7) if base_date else None
        # 该周任务总数（跨 kind），用于把任务均匀摊到本周 7 天（E1：周→日排期）
        week_count = sum(len(week_item.get(f) or []) for _, f in _KIND_FIELDS)

        order_index = 0
        for kind, field in _KIND_FIELDS:
            for raw_title in (week_item.get(field) or []):
                title = str(raw_title)
                # 按周内序把任务摊到 0..6 天，避免整周挤在首日（E1.1）
                day_offset = min(6, (order_index * 7) // week_count) if week_count else 0
                planned = week_start + timedelta(days=day_offset) if week_start else None
