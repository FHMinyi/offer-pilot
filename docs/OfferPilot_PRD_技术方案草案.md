...
> 🧭 **定位标注（2026-06-06 对齐）**：`docs/OfferPilot_原始想法.md`（原始想法）是本项目**唯一权威愿景基线**；本文是其**派生 PRD（v0.1）**，按 MVP 可落地角度收窄。本文所述"核心"均指**本期 MVP 范围的核心**，并非产品终极核心；与原始愿景的取舍见下方「§0 定位校准」与文末「**灵魂承载对照表**」。跟踪见 `docs/行动计划.md` 轨道 A。

## 0. 一页结论

> ⚖️ **定位校准（相对原始愿景）**：原始愿景（`OfferPilot_原始想法.md` 行101）把"简历匹配/日程规划"判为**表层**，把"**情绪管理与动态纠偏**"判为**真正的产品灵魂**。本 PRD 下文所说的"核心/要解决的"，指的是**本期 MVP 先落地的表层闭环的核心**，**不是产品的终极核心**。灵魂（动态纠偏/情绪兜底/人设引擎/陪伴/五阶段旅程）本期暂不做，但**列为下一里程碑的核心目标**（见文末§17 灵魂承载对照表），切勿因本文未展开而将其误当"非目标"永久丢弃。
...
> ⚖️ 注：以上是**本期 MVP 选定要先解决的问题**。原始愿景认为更深的痛点是"计划破产带来的情绪内耗"与"暗黑期缺乏陪伴"（痛点二、三），其解法（每日总结+实时调整+赛博导师陪伴）才是产品灵魂——本 PRD 暂不覆盖，见文末§17。
...
> 原始愿景（`OfferPilot_原始想法.md` 行101）明确："表层是简历匹配/日程规划，**真正的产品灵魂是情绪管理与动态纠偏**"。本 PRD 聚焦表层闭环，下列灵魂要素**本期 MVP 不做**——但**显式记录为后续里程碑核心**，避免被误当"非目标"永久丢弃。去向见 `docs/行动计划.md`（E=里程碑二、F=里程碑三）。

| 灵魂要素（原始愿景） | 愿景要求 | 本 PRD（MVP）处理 | 去向 |
|---|---|---|---|
| 动态纠偏（每日总结→实时重算，原文"最核心杀手锏"） | 今天没做完→自动降权、重组、重算明日日程 | ⛔ 未纳入；§7.5 仅一次性按周路线 | E1（M2，核心） |
| 情绪管理 / 情绪兜底 | 谷底期察觉中断、主动疏导、接住崩溃 | ⛔ 未纳入 | E2（M2，核心） |
| 动态人设引擎（教练/学长学姐/管家） | 人设矩阵；**原文明确 MVP 就要手动切换**；理智脑/情感脑解耦 | ⛔ 未纳入（§9/§10 仅理智脑节点） | E3（M2） |
| 赛博导师陪伴 / 稳定正反馈 | 永远在线的理性导师，正反馈缓解焦虑 | ⛔ §1.3"不是泛聊天机器人" + §10.2 多轮推迟 Phase2；陪伴层缺位 | E（M2） |
| 五阶段情绪旅程（触发/蜜月/谷底/碰壁/终局） | 数月马拉松 + 各阶段情绪触点介入 | ⛔ §5 仅单次线性流程 | C/E（M1/M2） |
| 四类用户画像差异化（在校/应届/转码/在职） | 各自策略与语气 | 🟡 §2 覆盖三类用户但无差异化策略 | F3（M3） |
